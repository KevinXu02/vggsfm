...
def run_one_scene(
    ...
    with autocast(dtype=dtype):
        if cfg.query_by_interval:
            ...
            query_frame_indexes = generate_rank_by_dino(
                reshaped_image, camera_predictor, frame_num
            )
    ...
    if cfg.center_order:
        ...
        if center_frame_index != 0:
            ...
            reshaped_image = switch_tensor_order([reshaped_image], center_order, dim=0)[
                0
            ]
    ...
    if cfg.dense_depth:
        ...
        unproj_dense_points3D = align_dense_depth_maps_and_save(
            reconstruction, sparse_depth, depth_dir, cfg
        )
...
def generate_rank_by_dino(
    ...
    rgbs = F.interpolate(
        reshaped_image, (image_size, image_size), mode="bilinear", align_corners=True
    )
